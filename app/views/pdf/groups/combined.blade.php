<html>
  ...
  <body style="font-size:13px">
     ...
     <table >
      <tr>
        ...
        <td>
          <strong>
              <h3>
              {{strtoupper($chama->name)}} GROUP COMBINED REPORT
              </h3>
           </strong>
        </td>
      </tr>
   ...
   <div class="content">
       <h3>{{strtoupper($chama->name)}} GROUP SAVING LISTING</h3>
       <hr>
      <table class="table table-bordered" style="width:100%">
        ...
        <tr>
          <td style="border-bottom:0.1px solid black; border-right:0.1px solid black;">
           ...
           <td style="border-bottom:0.1px solid black; border-right:0.1px solid black;">
               {{$loan->savingproduct->name}}
            </td>
           <td style="border-bottom:0.1px solid black; border-right:0.1px solid black;">
               {{$loan->account_number}}
            </td>
            <td style="border-bottom:0.1px solid black; border-right:0.1px solid black;">{{asMoney(Savingaccount::getAccountBalance($loan))}}
            </td>
        </tr>
        @endforeach
      </table>
     <br><br>
       <h3>{{strtoupper($chama->name)}} GROUP LOAN LISTING</h3>
       <hr>
       <table class="table table-bordered" style="width:100%">
        <tr>
          <td style="border-bottom:1px solid black;"><strong>Member #</strong></td>
        ...
        @foreach($loans as $loan2)
        <tr>
          <td style="border-bottom:0.1px solid black; border-right:0.1px solid black;">
              {{$loan2->member->membership_no}}
            </td>
           <td style="border-bottom:0.1px solid black; border-right:0.1px solid black;">
               {{$loan2->member->name}}
           </td>
           <td style="border-bottom:0.1px solid black; border-right:0.1px solid black;">
               {{$loan2->loanproduct->name}}
            </td>
           <td style="border-bottom:0.1px solid black; border-right:0.1px solid black;">
               {{$loan2->account_number}}
            </td>
            @if($loan2->is_approved==1)
            <td style="border-bottom:0.1px solid black; border-right:0.1px solid black;">
                APPROVED
            </td>
            @else
            ...
            @endif
            <td style="border-bottom:0.1px solid black; border-right:0.1px solid black;">{{asMoney(Loanaccount::getLoanAmount($loan2))}}
            </td>
           <td style="border-bottom:0.1px solid black; border-right:0.1px solid black;">{{asMoney(Loantransaction::getLoanBalance($loan2))}}
            </td>
        </tr>
        @endforeach
      </table>
