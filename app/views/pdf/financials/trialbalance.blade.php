   ...
   <div class="header">
     <table>
      <tr>
        ...
        <td>
          <strong><h3>TRIAL BALANCE AS FROM {{$fromDate}} TO {{$toDate}}</h3></strong>
        </td>
      </tr>
      <tr>
        <hr>
   ...
   <div class="content">
      <table class="table table-bordered" style="width:100%">
        ...
        <tr>
          ...
          <?php $total_debit = $total_debit + Account::getAccountBalanceAtDate($account, $fromDate,$toDate); ?>
          @endif
          @if($account->category == 'LIABILITY' || $account->category == 'INCOME' || $account->category == 'EQUITY')
          <td style="border-bottom:0.5px solid gray;">{{asMoney(0)}}</td>
          <td style="border-bottom:0.5px solid gray;">{{asMoney(Account::getAccountBalanceAtDate($account, $fromDate,$toDate))}}</td>
         <?php 
            $total_credit= $total_credit + Account::getAccountBalanceAtDate($account, $fromDate,$toDate); 
            ?>
          @endif
        </tr>
        @endforeach
