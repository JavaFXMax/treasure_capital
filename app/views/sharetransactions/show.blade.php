...
<tr>
    <td>Total Shares:</td>
    <td>
        {{ asMoney(Sharetransaction::where('type','=','credit')->sum('amount')-Sharetransaction::where('type','=','debit')->sum('amount'))}}
    </td>
</tr>
 </table>
</div>	
    ...
    <table id="users" class="table table-condensed table-striped table-responsive table-hover">
      ...
      <tbody>
        ...
        <tr>
          ...
          @endif
          <td>
            <a  href="{{ URL::to('sharetransactions/receipt/'.$transaction->id)}}" target="_blank"> <span class="glyphicon glyphicon-file" aria-hidden="true"></span>
                Receipt
            </a>
          </td>
        </tr>
