...
class Savingtransaction extends \Eloquent {
    ...
    public static function latePayment($savingaccount,$amount,$type){
        /*Create a new saving transaction*/
        $savingtransaction = new Savingtransaction;
		$savingtransaction->date = date('Y-m-d');
		$savingtransaction->savingaccount_id=$savingaccount;
		$savingtransaction->amount = $amount;
		$savingtransaction->type = $type;
		$savingtransaction->description = 'Late Payment Fee';
		...
		if($type == 'debit'){
			...
			foreach($savingpostings as $posting){
				if($posting->transaction == 'withdrawal'){
                    ...
                    $journal = new Journal;
                    $journal->journal_entry($data);
                    Savingtransaction::withdrawalCharges($savingaccount, date('Y-m-d'), $amount);
                    Audit::logAudit(date('Y-m-d'), Confide::user()->username, 'Late Payment Fee', 'Savings', $amount);
                }
            }
		}
    }
